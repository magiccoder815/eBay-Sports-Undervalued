...
base_url = "https://www.ebay.com/sch/i.html?_dcat=212&_udlo=100&_fsrp=1&_from=R40&_ipg=240&_pgn={}&Sport={}&_nkw=PSA&_sacat=64482&Grade=8%7C8%252E5%7C9%7C9%252E5%7C10%7C%21&Season=2020%7C2020%252D21%7C2021%7C2021%252D22%7C2022%7C2022%252D23%7C2023%7C2023%252D24%7C2024%7C2025&rt=nc&LH_Auction=1"
sport_name = "Baseball"
...
try:
    while True:
        ...
        # items = items_container.find_all('li', class_=re.compile(r'\bs-item\b.*\bs-item__pl-on-bottom\b'))
        items = items_container.find_all("li", attrs={"data-view": re.compile(r"^mi:1686\|iid:(\d+)$")})
        items_count = len(items)
        
        print(f"\nAuction items: {items_count}")
        
        if items_count < 239:
            print(f"\nFewer than 239 items found on this page: {items_count}.")
        ...
        for item in items:
            ...
            # Extract Title (removing "New Listing" if present)
            title = ""
            if title_element:
            ...
            if price_span:
                price_text = price_span.get_text(strip=True)  # Get the price text
                price = price_text  # You might want to further process this if necessary
            else:
                price = "N/A"  # Default value if price not found
        ...
        page_filename = os.path.join(sport_name, f"{sport_name}_Auction_Data_Page_{page}.xlsx")
        df.to_excel(page_filename, index=False)
        print(f"\nData for page {page} saved to '{page_filename}'.")
        
        page_end_time = time.time()
        page_duration = page_end_time - page_start_time
        print(f"\nTime taken for page {page}: {page_duration:.2f} seconds")
...
final_filename = os.path.join(sport_name, f"{sport_name}_Auction_Data.xlsx")
df_final.to_excel(final_filename, index=False)

# End timer and calculate duration
end_time = time.time()
execution_time = end_time - start_time
print(f"\Auction data saved to '{final_filename}'.")
